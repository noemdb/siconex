        ...
        @foreach($carreras as $carrera)
            ...
            <div class="card">
                <div class="card-header" id="heading_task_{{ $carrera->id or '' }}">
                    <h5 class="mb-0">
                        <button class="btn btn-link btn-sm" data-toggle="collapse" data-target="#accordion_task_{{ $carrera->id or '' }}" aria-expanded="true" aria-conttasks="accordion_task_{{ $carrera->id or '' }}">
                            <span class="text text-{{ $carrera->tipo or '' }}">{{ $carrera->fullname }}</span>
                        </button>
                    </h5>
                </div>
