...
class NivelsController extends Controller
{
    ...
    {
        return view('expediente.nivels.charts');
    }

    public function NivelsMonth(Request $request)
    {
        ...
        $month = Nivel::select(DB::raw('count(id) as value'),DB::raw('MONTH(created_at) as month'))
    ...
    public function NivelsMovs(Request $request)
    {
        $range = ($request->input('range')!=null) ? $request->input('range') : 'Todos';
        ...
        $dataSQL = Movimiento::select('nivels.codigo', DB::raw('count(movimientos.id) as value'))
        	->join('nivels','nivels.id','=','movimientos.nivel_id')
            ->Where('nivels.created_at', '>=', $finicial)
            ->Where('nivels.created_at', '<=', $ffinal)
            ->groupby('nivels.codigo')
            ->orderBy('nivels.codigo', 'asc')
            ->get()
            ->take($limit);
