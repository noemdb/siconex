...
$('.btn-delete').click(function (e) {
    ...
    if (result.value) {
        ...
        var id = row.data('documento');  //console.log(id);
        var form = $('#form-delete'); //console.log(form.attr('action'));
        var url = form.attr('action').replace(':DOCUMENTO_ID',id); //console.log(url);
        var data = form.serialize(); //console.log(data);

        $.post(url, data, function (result){
            row.fadeOut();
            var counter = $("#documento_counter").text() - 1;
            $("#documento_counter").text(counter);
            swal({
