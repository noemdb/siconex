...
$('.btn-delete').click(function (e) {
    ...
    if (result.value) {
        ...
        var id = row.data('task');  //console.log(id);
        var form = $('#form-delete'); //console.log(form.attr('action'));
        var url = form.attr('action').replace(':TASK_ID',id); //console.log(url);
        ...
        $.post(url, data, function (result){
            ...
            var task_counter = $("#task_counter").text() - 1;
            
            $("#task_counter").text(task_counter);
