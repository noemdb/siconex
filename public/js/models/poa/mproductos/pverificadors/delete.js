...
$('.btn-delete').click(function (e) {
    ...
    if (confirm("Estas seguro de realizar esta acción? Se eliminará definitivamente")) {
        ...
        var id = row.data('pverificador');  //console.log(id);
        var form = $('#form-delete'); //console.log(form.attr('action'));
        var url = form.attr('action').replace(':PINDICADOR_ID',id); //console.log(url);
        var data = form.serialize(); //console.log(data);

        $.post(url, data, function (result){            
            row.fadeOut();
            var pverificadors_counter = $("#pverificadors_counter").text() - 1;            
            $("#pverificadors_counter").text(pverificadors_counter);          
        }).fail(function () {
