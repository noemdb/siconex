...
$('.btn-delete').click(function (e) {
    ...
    if (confirm("Estas seguro de realizar esta acción? Se eliminará definitivamente")) {
        ...
        var id = row.data('pdeterminante');  //console.log(id);
        var form = $('#form-delete'); //console.log(form.attr('action'));
        var url = form.attr('action').replace(':PDETERMINANTE_ID',id); //console.log(url);
        var data = form.serialize(); //console.log(data);

        $.post(url, data, function (result){            
            row.fadeOut();
            var pdeterminantes_counter = $("#pdeterminantes_counter").text() - 1;            
            $("#pdeterminantes_counter").text(pdeterminantes_counter);            
        }).fail(function () {
