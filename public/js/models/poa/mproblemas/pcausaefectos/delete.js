...
$('.btn-delete').click(function (e) {
    ...
    if (confirm("Estas seguro de realizar esta acción? Se eliminará definitivamente")) {
        ...
        var id = row.data('pcausaefecto');  //console.log(id);
        var form = $('#form-delete'); //console.log(form.attr('action'));
        var url = form.attr('action').replace(':PCAUSAEFECTO_ID',id); //console.log(url);
        var data = form.serialize(); //console.log(data);

        $.post(url, data, function (result){            
            row.fadeOut();
            var pcausaefectos_counter = $("#pcausaefectos_counter").text() - 1;            
            $("#pcausaefectos_counter").text(pcausaefectos_counter);            
        }).fail(function () {
