...
$('.btn-delete').click(function (e) {
    ...
    if (confirm("Estas seguro de realizar esta acción? Se eliminará definitivamente")) {
        ...
        var id = row.data('presupuestaria');  //console.log(id);
        var form = $('#form-delete'); //console.log(form.attr('action'));
        var url = form.attr('action').replace(':MPRESUPUESTARIA_ID',id); //console.log(url);
        var data = form.serialize(); //console.log(data);

        $.post(url, data, function (result){            
            row.fadeOut();
            var presupuestarias_counter = $("#presupuestarias_counter").text() - 1;            
            $("#presupuestarias_counter").text(presupuestarias_counter);            
        }).fail(function () {
